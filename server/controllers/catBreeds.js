"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (g && (g = 0, op[0] && (_ = 0)), _) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
Object.defineProperty(exports, "__esModule", { value: true });
// Import dependencies
var express = require('express');
var bcrypt = require('bcrypt');
var CatBreed = require('../models/catBreed');
// Create router
var router = express.Router();
// GET cat breeds listing
router.get('/', function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
    var breeds, err_1;
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0:
                _a.trys.push([0, 2, , 3]);
                return [4 /*yield*/, CatBreed.find()];
            case 1:
                breeds = _a.sent();
                res.json(breeds);
                return [3 /*break*/, 3];
            case 2:
                err_1 = _a.sent();
                if (err_1 instanceof Error) {
                    res.status(500).json({ message: err_1.message });
                }
                return [3 /*break*/, 3];
            case 3: return [2 /*return*/];
        }
    });
}); });
// GET cat breed by id
router.get('/:id', getBreed, function (req, res) {
    res.json(res.breed);
});
// POST cat breed
router.post('/', function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
    var catBreed, newBreed, err_2;
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0:
                _a.trys.push([0, 2, , 3]);
                catBreed = new CatBreed({
                    name: req.body.name,
                    imageURL: req.body.imageURL,
                    origin: req.body.origin,
                    life_span: req.body.life_span,
                    temperament: req.body.temperament,
                    wikipedia_url: req.body.wikipedia_url,
                    weight: req.body.weight,
                    height: req.body.height,
                    adaptability: req.body.adaptability,
                    affection_level: req.body.affection_level,
                    child_friendly: req.body.child_friendly,
                    grooming: req.body.grooming,
                    intelligence: req.body.intelligence,
                    health_issues: req.body.health_issues,
                    social_needs: req.body.social_needs,
                    stranger_friendly: req.body.stranger_friendly,
                    vocalisation: req.body.vocalisation,
                    energy_level: req.body.energy_level,
                    description: req.body.description,
                });
                return [4 /*yield*/, catBreed.save()];
            case 1:
                newBreed = _a.sent();
                res.status(201).json(newBreed);
                return [3 /*break*/, 3];
            case 2:
                err_2 = _a.sent();
                if (err_2 instanceof Error) {
                    res.status(400).json({ message: err_2.message });
                }
                return [3 /*break*/, 3];
            case 3: return [2 /*return*/];
        }
    });
}); });
// Patch cat breed
router.patch('/:id', getBreed, function (req, res) {
    if (req.body.name != null) {
        res.catBreed.name = req.body.name;
        res.catBreed.origin = req.body.origin;
        res.catBreed.life_span = req.body.life_span;
        res.catBreed.temperament = req.body.temperament;
        (res.catBreed.wikipedia_url = req.body.wikipedia_url),
            (res.catBreed.weight = req.body.weight),
            (res.catBreed.height = req.body.height),
            (res.catBreed.adaptability = req.body.adaptability),
            (res.catBreed.affection_level = req.body.affection_level),
            (res.catBreed.child_friendly = req.body.child_friendly),
            (res.catBreed.grooming = req.body.grooming),
            (res.catBreed.intelligence = req.body.intelligence),
            (res.catBreed.health_issues = req.body.health_issues),
            (res.catBreed.social_needs = req.body.social_needs),
            (res.catBreed.stranger_friendly = req.body.stranger_friendly),
            (res.catBreed.vocalisation = req.body.vocalisation),
            (res.catBreed.energy_level = req.body.energy_level),
            (res.catBreed.description = req.body.description);
    }
    try {
        var updatedBreed = res.catBreed.save();
        res.json(updatedBreed);
    }
    catch (err) {
        if (err instanceof Error) {
            res.status(400).json({ message: err.message });
        }
    }
});
// Delete cat breed
router.delete('/:id', getBreed, function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
    var err_3;
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0:
                _a.trys.push([0, 2, , 3]);
                return [4 /*yield*/, res.breed.remove()];
            case 1:
                _a.sent();
                res.json({ message: 'Deleted cat breed' });
                return [3 /*break*/, 3];
            case 2:
                err_3 = _a.sent();
                if (err_3 instanceof Error) {
                    res.status(500).json({ message: err_3.message });
                }
                return [3 /*break*/, 3];
            case 3: return [2 /*return*/];
        }
    });
}); });
function getBreed(req, res, next) {
    return __awaiter(this, void 0, void 0, function () {
        var breed, err_4;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    _a.trys.push([0, 2, , 3]);
                    return [4 /*yield*/, CatBreed.findById(req.params.id)];
                case 1:
                    breed = _a.sent();
                    if (breed == null) {
                        return [2 /*return*/, res.status(404).json({ message: 'Cannot find breed' })];
                    }
                    return [3 /*break*/, 3];
                case 2:
                    err_4 = _a.sent();
                    if (err_4 instanceof Error) {
                        return [2 /*return*/, res.status(500).json({ message: err_4.message })];
                    }
                    return [3 /*break*/, 3];
                case 3:
                    res.name = breed;
                    next();
                    return [2 /*return*/];
            }
        });
    });
}
function authenticateCatBreed(req, res, next) {
    return __awaiter(this, void 0, void 0, function () {
        var breed, err_5, err_6;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    _a.trys.push([0, 2, , 3]);
                    return [4 /*yield*/, CatBreed.findOne({ name: req.body.name })];
                case 1:
                    breed = _a.sent();
                    if (breed == null) {
                        return [2 /*return*/, res.status(400).json({ message: 'Cannot find cat breed' })];
                    }
                    return [3 /*break*/, 3];
                case 2:
                    err_5 = _a.sent();
                    if (err_5 instanceof Error) {
                        return [2 /*return*/, res.status(500).json({ message: err_5.message })];
                    }
                    return [3 /*break*/, 3];
                case 3:
                    _a.trys.push([3, 5, , 6]);
                    return [4 /*yield*/, bcrypt.compare(req.body.password, breed.password)];
                case 4:
                    if (_a.sent()) {
                        res.breed = breed;
                        next();
                    }
                    else {
                        res.status(401).json({ message: 'Not allowed' });
                    }
                    return [3 /*break*/, 6];
                case 5:
                    err_6 = _a.sent();
                    if (err_6 instanceof Error) {
                        res.status(500).json({ message: err_6.message });
                    }
                    return [3 /*break*/, 6];
                case 6: return [2 /*return*/];
            }
        });
    });
}
module.exports = router;
